...
class PointNetSAModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for grouper, mlp in zip(self.groupers, self.mlps):
            features, femb = mlp(grouper(coords, centers_coords, femb, features))
            features_list.append(features.max(dim=-1).values)
        if len(features_list) > 1:
            return features_list[0], centers_coords, femb.max(dim=-1).values if femb.shape[1] > 0 else femb, o_mask
        else:
            return features_list[0], centers_coords, femb.max(dim=-1).values if femb.shape[1] > 0 else femb, o_mask
