...
class PVConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # print('pvconv', features.mean())
        voxel_features, voxel_coords = self.voxelization(features, coords)
        # print('pvconv', voxel_features.mean())
        voxel_features = self.voxel_layers(voxel_features)
        # print('pvconv', voxel_features.mean(), voxel_features.shape,voxel_coords.shape, self.resolution, self.training)
        voxel_features = F.trilinear_devoxelize(voxel_features, voxel_coords, self.resolution, self.training, o_mask)
        # print('pvconv', voxel_features.mean())
        fused_features = voxel_features + self.point_features(features)
        return fused_features, coords, femb, o_mask
...
class PVConvReLU(nn.Module):
    ...
    def forward(self, inputs):
        features, coords, femb = inputs
        voxel_features, voxel_coords = self.voxelization(features, coords)
        voxel_features = self.voxel_layers(voxel_features)
        voxel_features = F.trilinear_devoxelize(voxel_features, voxel_coords, self.resolution, self.training)
        fused_features = voxel_features + self.point_features(features)
        return fused_features, coords, femb
