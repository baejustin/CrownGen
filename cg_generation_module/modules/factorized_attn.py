...
class RPEAttention(nn.Module):
    ...
    def forward(self, x, temb, dentition_fdi_indices, attn_mask=None, attn_weights_list=None):
        out, attn = checkpoint(self._forward, (x, temb, dentition_fdi_indices, attn_mask), self.parameters(), self.use_checkpoint)
    ...
    def _forward(self, x, temb, dentition_fdi_indices, attn_mask):
        B, D, C, T = x.shape
        x = x.reshape(B*D, C, T)
        x = self.norm(x)
        x = x.view(B, D, C, T)
...
class FactorizedAttentionBlock(nn.Module):
    ...
    def __init__(self, channels, num_heads, use_rpe_net, time_embed_dim=None, use_checkpoint=False):
        ...
        self.intratooth_attention = RPEAttention(
    ...
    def forward(self, x, attn_mask, temb, T, dentition_fdi_indices=None, attn_weights_list=None):
        BT, C, P = x.shape
        B = BT//T
        ...
        x = self.intertooth_attention(x,
                                    ...
                                    attn_weights_list=None if attn_weights_list is None else attn_weights_list)

        # Now we attend over the intratooth dimensions. This is the number of points within a tooth.
        x = x.view(B, P, C, T).permute(0,3,2,1)  # B, T, C, P
        x = self.intratooth_attention(x,
                                   temb,
                                   dentition_fdi_indices=None)
        x = x.reshape(BT, C, P)
        return x
